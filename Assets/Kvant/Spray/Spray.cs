...
namespace Kvant
    ...
    public partial class Spray : MonoBehaviour
        {
        public static bool shooter = false;
        ...
        void UpdateKernelShader(bool isEmitting)
            {
            ...
            if (isEmitting == true)
                {
                if (shooter == true && _throttle != 0.6f)
                    {
        ...
        void ResetResources(bool isEmitting)
            {
            if (_bulkMesh == null)
            ...
            // Warming up
            InitializeAndPrewarmBuffers(isEmitting);

            _needsReset = false;
            }

        void InitializeAndPrewarmBuffers(bool isEmitting)
            {
            _noiseOffset = Vector3.zero;

            UpdateKernelShader(isEmitting);
        ...
        void Update()
            ...
            if (_needsReset) ResetResources(isEmitting);
